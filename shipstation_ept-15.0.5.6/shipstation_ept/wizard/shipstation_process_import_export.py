    ...
    def process_import_data(self, product_datas):
        ...
        for row_number, data in enumerate(product_datas):
            ...
            if data[3] == 0 or data[3] == '':
                self.create_log_line_for_map_product(log_book, model_id, row_number, 'Shipstation SKU missing')
                continue
            instance = self.env["shipstation.instance.ept"].search([('name', '=ilike', data[0])], limit=1)
            if not instance:
                self.create_log_line_for_map_product(log_book, model_id, row_number,
            ...
            if not shipstation_prod:
                ...
                self.create_log_line_for_map_product(log_book, model_id, row_number, msg)
                continue
            ...
            self.create_log_line_for_map_product(log_book, model_id, row_number, msg, product_id)
        log_book.unlink_log_book_without_log_lines()

    def create_log_line_for_map_product(self, log_book, model_id, count, msg, product_id=False):
        msg = 'Row: {}, {}'.format(count + 1, msg)
        _logger.info(msg)
        log_obj = self.env['common.log.lines.ept']
        vals = {
