...
class ShipstationCronConfigurationEpt(models.TransientModel):
    ...
    shipstation_validate_order_user_id = fields.Many2one('res.users', string="User for Validate Order",
                                                         ...
                                                         default=lambda self: self.env.user)
    # Cron for batch
    shipstation_auto_create_batch = fields.Boolean('Auto Create Batch', default=False,
                                                   help="Check if you want to Create Automatically Batch")
    shipstation_auto_create_batch_interval_number = fields.Integer('Interval Number for Auto Create Batch',
                                                                   help="Repeat every x.", default=1)
    shipstation_auto_create_batch_interval_type = fields.Selection([('minutes', 'Minutes'), ('hours', 'Hours'),
                                                                    ('days', 'Days'), ('weeks', 'Weeks'),
                                                                    ('months', 'Months')],
                                                                   'Interval Unit for Auto Create Batch')
    shipstation_auto_create_batch_next_execution = fields.Datetime('Next Execution for Auto Create Batch',
                                                                   help='Next Execution for Auto Create Batch')
    shipstation_auto_create_batch_user_id = fields.Many2one('res.users', string="User for Auto Create Batch",
                                                            help='User for Auto Create Batch',
                                                            default=lambda self: self.env.user)
    ...
    def onchange_shipstation_instance_id(self):
        ...
        self.update_auto_create_batch_cron_field(instance)

    def save(self):
        instance = self.shipstation_instance_id
        self.setup_shipstation_export_order_cron(instance)
        self.setup_shipstation_validate_order_cron(instance)
        self.setup_shipstation_auto_create_batch_cron(instance)
    ...
    def update_auto_create_batch_cron_field(self, instance):
        """
        :return:
        """
        try:
            auto_create_batch_cron_exist = instance and self.env.ref(
                'shipstation_ept.ir_cron_shipstation_auto_create_batch_instance_%d' % instance.id)
        except:
            auto_create_batch_cron_exist = False
        if auto_create_batch_cron_exist:
            self.shipstation_auto_create_batch = auto_create_batch_cron_exist.active or False
            self.shipstation_auto_create_batch_interval_number = auto_create_batch_cron_exist.interval_number or 1
            self.shipstation_auto_create_batch_interval_type = auto_create_batch_cron_exist.interval_type or False
            self.shipstation_auto_create_batch_next_execution = auto_create_batch_cron_exist.nextcall or False
            self.shipstation_auto_create_batch_user_id = auto_create_batch_cron_exist.user_id.id or False

    def setup_shipstation_auto_create_batch_cron(self, instance):
        """
        :return:
        """
        try:
            ...
            cron_exist = False
        if self.shipstation_auto_create_batch:
            nextcall = datetime.now() + _intervalTypes[self.shipstation_auto_create_batch_interval_type](
                self.shipstation_auto_create_batch_interval_number)
            vals = self.prepare_val_for_cron(self.shipstation_auto_create_batch_interval_number or 1,
                                             self.shipstation_auto_create_batch_interval_type,
                                             self.shipstation_auto_create_batch_user_id)
            vals.update(
                {'nextcall': self.shipstation_auto_create_batch_next_execution or nextcall.strftime(
                    '%Y-%m-%d %H:%M:%S'),
                 'code': "model.create_batch_using_cronjob(ctx={'shipstation_instance_id':%d})" % instance.id,
                 })
            if cron_exist:
                vals.update({'name': cron_exist.name})
                cron_exist.write(vals)
                ...
                try:
                    core_cron = self.env.ref("shipstation_ept.ir_cron_shipstation_auto_create_batch")
                except:
                    core_cron = False
                name = instance.name + ' : ' + core_cron.name
                vals.update({'name': name})
                new_cron = core_cron.copy(default=vals)
                name = 'ir_cron_shipstation_auto_create_batch_instance_%d' % (instance.id)
                self.create_ir_module_data_for_export(name, new_cron)
        else:
            if cron_exist:
                cron_exist.write({'active': False})
