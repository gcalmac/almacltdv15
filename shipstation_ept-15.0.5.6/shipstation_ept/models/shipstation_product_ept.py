...
class ShipstationProduct(models.Model):
    ...
    def import_product(self, instance, auto_create_product, product_start_date='',
        ...
        log_book = self.env['common.log.book.ept'].create_shipstation_log('import', model_id)
        ...
        while True:
            ...
            for res in response.get('products', []):
                ...
                if not product_id:
                    log_book.create_log_book_line_for_shipstation("ProductId not found in Response from API", model_id)
                    continue

                if not sku:
                    log_book.create_log_book_line_for_shipstation("SKU not found in Response from API", model_id)
                    continue
                ...
                if product_id and sku:
                    ...
                    if existing_shipstation_product and \
                        ...
                        log_book.create_log_book_line_for_shipstation("Shipstation Product for {} sku Adding shipstation Id.".format(sku), model_id)
                    ...
                    if odoo_product:
                        ...
                        log_book.create_log_book_line_for_shipstation(
                            "Shipstation Product for {} sku created".format(sku), model_id, shipstaion_product_id)
                    else:
                        if not auto_create_product:
                            log_book.create_log_book_line_for_shipstation(
                                "Internal referance {} not found in ERP".format(sku), model_id)
                        else:
                            ...
                            log_book.create_log_book_line_for_shipstation(
                                "Odoo Product for {} sku created".format(sku), product_model_id, product)
                            ...
                            log_book.create_log_book_line_for_shipstation(
                                "Shipstation Product for %s sku created" % sku, product_model_id, product)
            querystring['page'] += 1
