...
class WebsiteStockConfigSettings(models.Model):
    ...
    save_setting_on_product = fields.Boolean(string="Save Current Setting on Product", help="True value of this field save 'allow/deny order' setting on the product variants if you uncheck this box then product variants setting not overwrite by the main setting.", default=True)

    @api.depends('wk_stock_location')
    def _get_warehouse_id(self):
        if self.wk_stock_location:
            self.wk_warehouse_name = self.wk_stock_location.warehouse_id.id
