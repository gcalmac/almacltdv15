...
class ProductTemplate(models.Model):
    ...
    def wk_get_product_vatiants(self):
        ...
        action = self.env.ref('product.product_variant_action')
        list_view_id = self.env.ref('product.product_product_tree_view')
        form_view_id = self.env.ref('website_stock.Wk_website_stock_product')
        result = {
            'name': action.name,
            'help': action.help,
            'type': action.type,
...
class Website(models.Model):
    ...
    def get_product_stock_qty(self, product_obj, type_stock):
        ...
        if product_obj.type in ['service', 'consu']:
            return 10
